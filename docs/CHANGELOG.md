...
- On Windows, fall back to reading `eagle.clipboard.readImage()` directly when format probing does not report an image. This targets `Win+Shift+S` screenshots that enter the clipboard but are not detected by `eagle.clipboard.has(...)`.
